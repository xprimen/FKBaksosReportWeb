...
export default function Page({ params: { seq } }: { params: { seq: string } }) {
  const { state, dispatch } = useContext(AppContext);
  const { push } = useRouter();
  const row = useMemo(
    () =>
      state.transaksi.data.filter((d: TTransaksi) => String(d.seq) === seq)[0],
    [seq, state.transaksi.data]
  );
  ...
  const onSubmit: SubmitHandler<TTransaksi> = async (data) => {
    ...
    //   { shallow: true }
    // );
    push("/?notif=" + encodeURIComponent("Berhasil Disimpan"));
  };

  useEffect(() => {
    setValue("tgl_iuran", row.tgl_iuran);
  }, [row.tgl_iuran, setValue]);
